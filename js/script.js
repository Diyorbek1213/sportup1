...
if (window.innerWidth > 920) {
	...
	const cartLogo = document.querySelector('.cart__logo');
	...
	headerCart.addEventListener('mouseover', () => {
		cartItems.classList.add('_active');
	})
	bgVideo.addEventListener('mouseover', () => {
		cartItems.classList.remove('_active');
	})
...
if (mainMenu && menuContent && header && mainContainer) {
	mainMenu.addEventListener('mouseenter', (e) => {
		setTimeout(() => {
			...
			// })
			menuBack.addEventListener('mouseenter', () => {
...
// searchPrompt.addEventListener('mouseleave', () => {
// 	searchPrompt.classList.remove('_active');
// })
